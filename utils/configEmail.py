...
class Email:
    ...
    def config_file(self):
        ...
        if self.check_file():
            ...
            zipfile_name = str(datetime.now().strftime("%Y%m%d%H%M")) + '.zip'
            zippath = os.path.join(readConfig.proDir, "result", zipfile_name)

            self.log.build_out_info_line('Start to zip.' + zippath)
            # f = zipfile.ZipFile(zippath, 'w', compression=zipfile.ZIP_DEFLATED)
            f = zipfile.ZipFile(zippath, 'w', compression=zipfile.ZIP_STORED)
            pre_len = len(os.path.dirname(reportpath))

            for dirpath, dirnames, filenames in os.walk(reportpath):
    ...
    def send_email(self):
        ...
        try:
            ...
            smtp.quit()
            self.log.build_out_info_line("The test report has send to developer by email.")
        except:
